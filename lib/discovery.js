...
function onDiscoveryIBeacon(peripheral) {
  ...
  if (entered) {
    ...
    mqtt.send(`${config.mqtt_prefix}/device_tracker/ble-${id}-tracker/state`, home?"home":"not_home", {retain: true});
  }

  let mqttData = { distance: distance, rssi: peripheral.rssi, last_updated_at: (new Date()).toISOString() };
